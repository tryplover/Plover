...
### pnpm install in a git worktree: TTY abort, then a cross-worktree file lock
**Symptom:** Two failures in a row when installing into a `.claude/worktrees/<name>` checkout after merging a branch whose lockfile moved. First `ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY  Aborted removal of modules directory due to no TTY`. Setting `CI=true` gets past that, and it then fails with `EPERM: operation not permitted, unlink '<worktree>/node_modules/.pnpm/@esbuild+win32-x64@<ver>/node_modules/@esbuild/win32-x64/esbuild.exe'` — naming a path inside the *worktree*, even though nothing in the worktree is running.
**Root cause:** Two separate things. (1) When the lockfile has diverged enough, pnpm wants to recreate `node_modules` wholesale and asks for confirmation; a non-interactive shell has no TTY to answer, so it aborts — and crucially `typecheck`/`lint` afterwards still pass against the **stale** tree, a false green. (2) pnpm hardlinks binaries from its content-addressable store, so the worktree's `esbuild.exe` and the primary checkout's are the same file on disk. A `pnpm dev` session running in the *primary* checkout holds an open handle, and on Windows that blocks deletion through any hardlink to it.
**Fix:** Run worktree installs with `CI=true` (PowerShell: `$env:CI = 'true'`), and treat any `typecheck`/`lint` result after an aborted install as meaningless — re-run them only once install exits 0. For the EPERM, check `Get-Process -Name node,pnpm,esbuild,electron | Select-Object Id,Name,Path` — the `Path` column tells you which checkout owns the lock. Ask the user before stopping a dev session; the lock is only needed for the install itself, so they can restart it as soon as install finishes.

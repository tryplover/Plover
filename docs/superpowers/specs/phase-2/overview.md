...
Phase 2 expands Plover from "type a goal, get it scheduled locally" (Phase 1) into a context-aware productivity agent that observes what the user is doing and pulls / pushes work across the tools they already use.
...
1. **Screen tracking completion** — [features/screen-tracking.md](./features/screen-tracking.md)
...
- **Inference / Nudge** (still stubbed from Phase 1) become live consumers of the richer activity stream.
- **Backend proxy** (`plover-server`) gains endpoints for any API the renderer must not hold credentials for: GitHub OAuth callback, Notion OAuth callback, Gemini Vision for screenshots.
...
3. **Permissions on macOS** — Phase 2 introduces new permission asks, each gated behind an explicit user opt-in:
   - **Screen Recording** — required only when the user enables screenshot capture.
   - **Full Disk Access** — required only when the user enables file indexing.
...
1. **Screen tracking completion.** Finish the existing tracking subsystem with screenshot capture (opt-in), advanced window metadata, an activity-timeline UI in the renderer, and planner context plumbing.
2. **GitHub integration.**
3. **Drive write-back + Sheets.** Reuses existing Google OAuth.
4. **Notion integration.** New OAuth provider.
5. **File access (Full Disk Access).** Big trust ask.
...
3. Every new event kind in `activity.kind` is rendered by the activity timeline view.
4. `pnpm typecheck && pnpm lint && pnpm test` is clean after each step.
5. `keytar` stores every OAuth token; no token ever lands in SQLite.

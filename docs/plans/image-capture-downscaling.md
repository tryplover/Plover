...
- [ ] Add `const VISION_UPLOAD_MAX_WIDTH = 1024;` near the top of the file.
- [ ] Change `captureOnce()`'s call site so `runInference` also receives the
      ...
      for nothing else).
- [ ] Update `runInference`'s signature to accept the `NativeImage` + original
...
- [ ] Double-check: the `fs.writeFile(filePath, png)` call in `captureOnce` and
...
- [ ] Add a test where the mocked capture is wider than `VISION_UPLOAD_MAX_WIDTH`
...
- [ ] Add a test where the mocked capture is already `<= VISION_UPLOAD_MAX_WIDTH`
      (e.g. `{ width: 800, height: 600 }`) and assert `thumbnail.resize` is
      **not** called, and the upload body uses the original buffer unchanged.
- [ ] Run the full existing suite in this file to make sure nothing about the

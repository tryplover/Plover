...
You MUST call the tool "matchCommit" with the result.`;
    ...
    if (!call) {
      return res.status(502).json({ error: 'Gemini failed to call the matchCommit function' });
    }
    if (call.name !== 'matchCommit') {
      return res.status(502).json({ error: `Unexpected function call from Gemini: ${call.name}` });
    }

    const args = call.args as unknown as MatchCommitArgs;
    if (!args || typeof args.matchedTaskId !== 'string') {
      return res.status(502).json({ error: 'Invalid arguments returned in matchCommit function call' });
    }

    const reasoning = typeof args.reasoning === 'string' ? args.reasoning.trim() : '';
    const validIds = new Set(tasks.map((t: { id: string }) => t.id));
    const matchedTaskId =
      args.matchedTaskId === 'null' || !validIds.has(args.matchedTaskId) ? null : args.matchedTaskId;

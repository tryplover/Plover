...
describe('Injection Mitigation', () => {
  ...
  describe('/api/decompose', () => {
    it('sanitizes and frames recentActivity', async () => {
      ...
      // Nesting should be allowed up to depth 4
      expect(capturedPrompt).toContain('too deep');
    });
  });

  describe('Sanitization Edge Cases', () => {
    it('handles non-string windowContext fields', async () => {
      let capturedPrompt = '';
      generateContent.mockImplementationOnce(async (req: any) => {
      ...
      const res = await post(server, '/api/infer-screen', {
        ...
        windowContext: { app: 123, title: null, browserUrl: undefined },
      });

      expect(res.status).toBe(200);
      expect(capturedPrompt).toContain('app=""');
      expect(capturedPrompt).toContain('title=""');
      expect(capturedPrompt).not.toContain('url=');
    });

    it('enforces depth limit of 4', async () => {
      let capturedPrompt = '';
      generateContent.mockImplementationOnce(async (req: any) => {
        capturedPrompt = req.contents[0]?.parts[0]?.text ?? '';
        return {
      ...
      expect(res.status).toBe(200);
      expect(capturedPrompt).toContain('at limit');

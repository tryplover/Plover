import { describe, expect, it, beforeEach, vi } from 'vitest';

vi.mock('keytar');
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/test'),
  },
}));

import Database from 'better-sqlite3';
import { runMigrations } from '../../src/main/store/db';
import { ActivityRepo } from '../../src/main/store/repos/activity';
import { SettingsRepo } from '../../src/main/store/repos/settings';
import { TypedEventBus } from '../../src/main/events/bus';
import { GitHubCommitActivitySubscriber } from '../../src/main/activity/sources/github/github-commit-subscriber/github-commit-subscriber';

describe('GitHubCommitActivitySubscriber', () => {
  let db: Database.Database;
  let activity: ActivityRepo;
  let settings: SettingsRepo;
  let bus: TypedEventBus;

  beforeEach(() => {
    vi.clearAllMocks();
    db = new Database(':memory:');
    runMigrations(db);
    activity = new ActivityRepo(db);
    settings = new SettingsRepo(db);
    bus = new TypedEventBus();
  });

  it('writes a github_commit activity row on github.commit when enabled', () => {
    settings.update({ githubTrackingEnabled: true, pauseAllTracking: false });
    new GitHubCommitActivitySubscriber(activity, settings, bus).start();
    bus.emit('github.commit', {
      repo: 'o/r',
      sha: 'abc123',
      message: 'Fix bug',
      author: 'octocat',
      url: 'https://github.com/o/r/commit/abc123',
      committedAt: '2026-08-04T00:00:00.000Z',
    });
    const rows = activity.list({ kind: 'github_commit' });
    expect(rows).toHaveLength(1);
    expect(rows[0]?.payload).toMatchObject({ sha: 'abc123', repo: 'o/r' });
  });

  it('does not write when pauseAllTracking is set', () => {
    settings.update({ githubTrackingEnabled: true, pauseAllTracking: true });
    new GitHubCommitActivitySubscriber(activity, settings, bus).start();
    bus.emit('github.commit', {
      repo: 'o/r',
      sha: 'abc123',
      message: 'Fix bug',
      author: 'octocat',
      url: 'https://github.com/o/r/commit/abc123',
      committedAt: '2026-08-04T00:00:00.000Z',
    });
    expect(activity.list({ kind: 'github_commit' })).toHaveLength(0);
  });
});
